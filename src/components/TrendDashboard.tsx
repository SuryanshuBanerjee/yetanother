...
import { TrendingUp, TrendingDown, Share2, Check, Copy } from "lucide-react";
...
interface TrendDashboardProps {
    data: {
        ...
        newsArticles?: { title: string; description: string; source: string; date: string; image?: string; url?: string }[];
...
export default function TrendDashboard({ data, onRelatedTrendClick, userRole = "general-user" }: TrendDashboardProps) {
    ...
    const handleShare = async () => {
        const shareText = [
            `TREND PRISM: ${keyword}`,
            `Verdict: ${verdictText}`,
            `Phase: ${phase} | WoW: ${weekChange >= 0 ? "+" : ""}${weekChange.toFixed(1)}%`,
            verdict?.confidence ? `Confidence: ${verdict.confidence}%` : "",
            verdict?.riskLevel ? `Risk: ${verdict.riskLevel}` : "",
            summaryText ? `\n${summaryText}` : "",
            `\nGenerated by TREND PRISM — AI Trend Decline Prediction`,
        ].filter(Boolean).join("\n");

        if (navigator.share) {
            try {
                await navigator.share({ title: `TREND PRISM: ${keyword}`, text: shareText });
                return;
        ...
        >
            ...
            >
                ...
                <div className="flex gap-3">
                    <button
                        onClick={handleShare}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-sm transition-colors border border-white/10"
                    >

...
async function fetchInterestOverTime(keyword: string) {
    try {
        const data = await serpTrends({ q: keyword, data_type: "TIMESERIES", date: "today 3-m" }) as {
            interest_over_time?: { timeline_data: Array<{ timestamp: string; date: string; values: Array<{ extracted_value: number; value: string }> }> };
        };
        return (data.interest_over_time?.timeline_data ?? []).map((item) => ({
            time: item.timestamp,
            formattedTime: item.date.split("–")[0].trim(),
            value: [item.values[0]?.extracted_value ?? 0],
            formattedValue: [item.values[0]?.value ?? "0"],
        }));
    } catch (e) {
...
async function fetchInterestByRegion(keyword: string) {
    try {
        const data = await serpTrends({ q: keyword, data_type: "GEO_MAP_0", date: "today 3-m" }) as {
            interest_by_region?: Array<{ geo: string; location: string; extracted_value: number; value: string }>;
        };
        return (data.interest_by_region ?? []).map((item) => ({
            geoCode: item.geo,
            geoName: item.location,
...
async function fetchRelatedQueries(keyword: string) {
    try {
        const data = await serpTrends({ q: keyword, data_type: "RELATED_QUERIES", date: "today 3-m" }) as {
            related_queries?: {
        ...
        const toItem = (q: { query: string; extracted_value: number; value: string; link: string }) => ({
            query: q.query,
            value: q.extracted_value,
            formattedValue: q.value,
            link: q.link ?? "",
        });
        return {
            top: (data.related_queries?.top ?? []).slice(0, 10).map(toItem),
            rising: (data.related_queries?.rising ?? []).slice(0, 10).map(toItem),
        };
    } catch (e) {
